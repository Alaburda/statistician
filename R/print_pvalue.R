...
format_p <- function(pvalues, decimals = 3, numbers_only = FALSE, decimal_seperator = ",") {
  return(vectorise_print(pvalues, decimals, format_p_, numbers_only))
}
...
format_p_ <- function(pvalue, decimals, numbers_only) {
  ...
  if (pvalue >= 0.001) {
    p <- paste0("$p = ", gsub("\\.",decimal_seperator,force_decimals(pvalue, decimals)), "$")
  }
  ## Special case: p-value is 1 (or the rounded p-value is 1)
  if (round(pvalue, decimals) == 1) {
    p <- paste0("$p > 0",decimal_seperator, paste0(rep("9", decimals), collapse = ""), "$")
  }
  if (pvalue < 0.01 & decimals <= 2) {
    p <- paste0("$p < 0",decimal_seperator,"01$")
  }
  if (pvalue < 0.001) {
    p <- paste0("$p < 0",decimal_seperator,"001$")
  }
  if (numbers_only) {
    p <- gsub("p = ", "", p)
    p <- gsub("p ", "", p)
