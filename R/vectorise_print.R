...
force_or_cut <- function(x, decimals = 2) {
  return(vectorise_print(x, decimals, force_or_cut_))
}
...
## An abstract function used to vectorise all number printing functions
vectorise_print <- function(x, decimals, FUN, ...) {
  x <- as.numeric(x)
